...
class ProtoSpiderPipeline(object):
    settings = None
    es = None
    items_buffer = []
    ...
    def init_es_client(cls, crawler_settings):
        ...
        es_servers = crawler_settings.get('ELASTICSEARCH_SERVERS', 'https://search-search-cabinvista-umu7macroaqetldvevrj7pegvm.us-east-2.es.amazonaws.com:9200')
        ...
        index_mapping = crawler_settings['ELASTICSEARCH_TYPE_MAPPING']

        es_index_name= crawler_settings['ELASTICSEARCH_INDEX']
        #Delete current index and replace with a new one
        ...
        #Make new index it is being made for the first time
        else:
            es.indices.create(index=es_index_name, body=index_mapping)
    ...
    def process_item(self, item, spider):
        if isinstance(item, types.GeneratorType) or isinstance(item, list):
            ...
            return item
    #Creates duplicates in elasticsearch index
